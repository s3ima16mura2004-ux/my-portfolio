...
// ============================================================
let monthlyChallengeMonthKey = "";  // 現在追跡中の月（"YYYY-M"形式）
let monthlyChallengeKey = "";       // 今月選ばれているチャレンジのkey
let monthlyChallengeProgress = 0;   // 今月のチャレンジの進捗
let monthlyChallengeClaimed = false; // 今月のチャレンジの受取済みフラグ
...
// （state.jsに置くことで、ui-shop.jsを読み込んでいない軽量ページからも同じ表示を呼び出せるようにしている）
function updateEquippedBadgesDisplay() {
    const display = document.querySelector("#equipped-title-display");
    if (!display) return;

    const avatarPart = (equippedAvatarKey && equippedAvatarEmoji) ? equippedAvatarEmoji + " " : "";
    const titlePart = (equippedTitleKey && equippedTitleEmoji) ? equippedTitleEmoji + " " + equippedTitleName + " " : "";

    if (avatarPart || titlePart) {
        display.textContent = avatarPart + titlePart;
...
// 👒 アバターアクセサリーを購入する
async function buyAvatarItem(key) {
    const item = AVATAR_ITEMS.find(i => i.key === key);
    if (!item) return;
    if (ownedAvatarItems[key]) return;
    if (currentMoney < item.price) {
        alert("🙅 所持金が足りません！\n必要：" + item.price.toLocaleString() + "円");
        return;
    }

    currentMoney -= item.price;
    ownedAvatarItems[key] = true;

    if (typeof updateMoneyDisplay === "function") updateMoneyDisplay();
    if (typeof updateAvatarShopUI === "function") updateAvatarShopUI();
    if (typeof playSE === "function") playSE("se-purchase");
    await saveUserState();

    if (typeof showToast === "function") {
        showToast("👒 「" + item.emoji + item.name + "」を購入しました！", "success");
    } else {
        alert("👒「" + item.emoji + item.name + "」を購入しました！");
    }
}

// 👒 アバターアクセサリーを装備する（すでに装備中のものをもう一度選ぶと装備解除）
async function equipAvatarItem(key) {
    if (key && !ownedAvatarItems[key]) return;

    if (equippedAvatarKey === key) {
        equippedAvatarKey = "";
        equippedAvatarEmoji = "";
    } else {
        const item = AVATAR_ITEMS.find(i => i.key === key);
        if (!item) return;
        equippedAvatarKey = key;
        equippedAvatarEmoji = item.emoji;
    }
...
async function saveUserState() {
    if (!currentUser || !window.omikujiDB || !window.omikujiDoc || !window.omikujiUpdateDoc) return false;
    try {
        await window.omikujiUpdateDoc(window.omikujiDoc(window.omikujiDB, "users", currentUser), {
            ...
            equippedAvatarKey: equippedAvatarKey,
            ...
            duelChallengesIncoming: duelChallengesIncoming,
            duelChallengesOutgoing: duelChallengesOutgoing,
            ...
            monthlyChallengeMonthKey: monthlyChallengeMonthKey,
            monthlyChallengeKey: monthlyChallengeKey,
            monthlyChallengeProgress: monthlyChallengeProgress,
            monthlyChallengeClaimed: monthlyChallengeClaimed,
...
// 👒 アバターアクセサリー
let ownedAvatarItems = {};    // 所持しているアクセサリー（{ "アイテムkey": true, ... }）
let equippedAvatarKey = "";   // 装備中のアクセサリーのkey
let equippedAvatarEmoji = ""; // 装備中のアクセサリーの絵文字（表示の高速化用にキャッシュ）
...
// 🎲 サイコロ対決（フレンド対戦）：{ "相手の名前": { bet: 賭け金 } } の形で保持する
let duelChallengesIncoming = {};   // 自分宛に届いている対戦の挑戦状
let duelChallengesOutgoing = {};   // 自分が送った対戦の挑戦状（承認待ち）
...
async function loadUserState() {
    ...
    if (snap.exists()) {
        ...
        ownedAvatarItems = (data.ownedAvatarItems && typeof data.ownedAvatarItems === "object") ? data.ownedAvatarItems : {};
        equippedAvatarKey = data.equippedAvatarKey || "";
        equippedAvatarEmoji = data.equippedAvatarEmoji || "";
        ...
        duelChallengesIncoming = (data.duelChallengesIncoming && typeof data.duelChallengesIncoming === "object") ? data.duelChallengesIncoming : {};
        duelChallengesOutgoing = (data.duelChallengesOutgoing && typeof data.duelChallengesOutgoing === "object") ? data.duelChallengesOutgoing : {};
        ...
        monthlyChallengeMonthKey = data.monthlyChallengeMonthKey || "";
        monthlyChallengeKey = data.monthlyChallengeKey || "";
        monthlyChallengeProgress = typeof data.monthlyChallengeProgress === "number" ? data.monthlyChallengeProgress : 0;
        monthlyChallengeClaimed = data.monthlyChallengeClaimed === true;
