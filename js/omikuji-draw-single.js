...
function omikuji() {
    ...
    function determineResult() {
        ...
        if (usedBirthdayTicket) {
            ...
            if (kodomonohiActive) gotKodomonohiExtreme = true; // 🎏 こどもの日の称号判定
        } else if (rawOkj >= daidaikichiThreshold) {
            // ☀️ 激レア「大大吉」
            // 🛡️ ここも神吉と同様、大吉運ボーナス適用前の生の乱数(rawOkj)で判定し、積み上げの影響を受けないようにしている
            isExtremeTier = true;
            imgSrc = "../images/omikuji_daidaikichi.jpg";
            resultName = "大大吉";
            prizeMoney = DAIDAIKICHI_PRIZE;
        ...
        } else if (resultName === "大大吉") {
            playSE("se-daidaikichi");
            startConfetti();
            startConfetti();
            if (isNatsumatsuriFestivalActive()) startFireworks();
        } else if (resultName === "大吉" || resultName === "吉" || resultName === "中吉") {
            playSE("se-win");
            if (resultName === "大吉") {
                startConfetti();
                if (isNatsumatsuriFestivalActive()) startFireworks();
