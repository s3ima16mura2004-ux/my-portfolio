...
async function saveUserState() {
    ...
    try {
        await window.omikujiUpdateDoc(window.omikujiDoc(window.omikujiDB, "users", currentUser), {
            ...
            japanShrinesOwned: japanShrinesOwned
        });
    } catch (e) {
...
let japanShrinesOwned = {};   // 参拝済みの神社（{ 神社key: true, ... }）
...
// 🗾 参拝済みの神社の総数（コンプリート状況の表示・称号判定に使用）
function getJapanShrineOwnedCount() {
    return Object.keys(japanShrinesOwned).filter(k => japanShrinesOwned[k]).length;
}

// 🗾 指定した都道府県の神社をすべて参拝済みかどうか
function isJapanPrefectureComplete(pref) {
    return pref.shrines.every(s => japanShrinesOwned[s.key]);
}

// 🗾 コンプリート（神社をすべて参拝済み）した都道府県の数
function getJapanPrefectureCompleteCount() {
    return JAPAN_PREFECTURES.filter(isJapanPrefectureComplete).length;
}

// 🗾 全国神社巡りマップをすべて埋め終えたかどうか（全都道府県の全神社を参拝済み）
