...
async function saveUserState() {
    ...
    try {
        await window.omikujiUpdateDoc(window.omikujiDoc(window.omikujiDB, "users", currentUser), {
            ...
            shrineMapJapanLevel: shrineMapJapanLevel
        });
    } catch (e) {
...
// 🗾 全国神社巡りマップ関連の状態（境内マップ完成後に解放される第2段階）
let shrineMapJapanLevel = 0; // 巡り終えた都道府県の数（0〜MAP_TILES_JAPAN.length）。北海道から沖縄へ順番に進む
...
// 🗾 全国神社巡りマップをすべて埋め終えたかどうか
function isShrineMapJapanComplete() {
    return shrineMapJapanLevel >= MAP_TILES_JAPAN.length;
}